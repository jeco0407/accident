...
**有 transition 的屬性不能點完馬上量。**（v55 擴稽核時我又踩了一次 —— `#loc-card` 的背景過渡是 0.45s，等 0.35s 量到的是還沒變暗的白底，深色模式整批誤報。所以現在 `a11y.py` 直接把過渡關掉，不再靠等。） `.role-opt` 的 background 與 border-color 有 120ms 過渡，click 之後同一個 tick 讀 `getComputedStyle`，拿到的是**動畫起點**，也就是還沒選中的顏色。看起來就像 CSS 規則沒生效 —— 我為此去翻了特異性、`!important`、UA 樣式，全都不是。量之前先 `setTimeout` 個 300～500ms。同一張卡上沒有宣告 transition 的 `.tick` 當下就正確，那個不一致就是線索。

## 稽核的盲點（v55 補上了，但教訓要留著）

v55 之前 `a11y.py` 只走五個**分頁**，十一個覆蓋層一個都沒掃。歷史上出過的兩次無障礙問題都在那裡面 —— v34 的 `.sheet-close` 40×38、v46 的裁切取景框。**腳本沒報錯不等於沒問題，只等於它沒走到那裡。**

擴完之後第一次跑就撈到五個，其中兩個是真的會影響使用的：深色模式下刪除確認鍵是 2.29:1、請求明細是白紙配淺灰字 1.96:1 —— 後者是前一版才寫的東西。

擴的時候踩到三個坑，都值得記：

- **覆蓋層打開時，底下那一頁在 DOM 裡仍然是可見的。** 不限定根元素的話，每一屏都會把整個 App 再量一次，重複幾百項而且標籤會騙人。
- **`bgOf()` 不能無視 alpha。** 這個 App 的 `--inset` 是 `rgba(255,255,255,.05)`，只取前三個數字就會把深色卡片上那層 5% 白讀成純白，整批誤報成 1.9:1。要沿著祖先鏈合成到不透明為止。
- **切完主題不能馬上量**（見下一節）。修法不是等更久，是注入一段把全站 `transition`／`animation` 關掉的樣式 —— 過渡只影響中間狀態，最終計算值一模一樣。

另外 `aria-hidden` 的子樹要跳過：那是刻意不給輔助科技看的東西，量它只會製造雜訊。

...
第一次定位就把座標與時間鎖成**事故地點**寫進 localStorage（`aa.site.v1`），之後 `watchPosition` 的更新只用來算「離事故地點多遠」，**不會覆蓋**它。處理事故時人常會移動（移車、進超商等對方），若讓即時定位直接覆寫，事發位置就被無聲地蓋掉了。

距離超過 40 公尺才顯示目前位置那一列 —— GPS 本來就有數公尺誤差，在原地時顯示距離只是雜訊。要改地點得按「改設為事故地點」並確認。

地址用 Nominatim 的**結構化欄位**組（縣市 → 鄉鎮市區 → 路 → 號），不是反轉 `display_name`。台灣的欄位對應不直覺：直轄市的「區」在 `suburb`，縣的「鄉鎮」在 `town`，而 `city_district` 在直轄市是「里」、在縣是「村」，兩者都是雜訊。地址只查一次，存進事故地點裡。

## 存證資料存在哪裡

- **事故地點與時間**：localStorage
- **對方資料**：localStorage

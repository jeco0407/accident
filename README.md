...
`contactUrl` 是現場、出險、求償三頁清單下方「還是不知道怎麼做嗎？歡迎聯絡我們」，以及「設定」頁自我介紹卡「用 LINE 找我」的連結，留空則不顯示。自我介紹的文字直接寫在 `index.html` 的 `.about` 卡片裡。客戶一律聯絡官方帳號，App 內沒有個別業務員的聯絡方式。
...
- **使用者自己的資料**：v65 起不存。求償明細的請求人欄印出來手寫
...
- `privacy.html` — 隱私權政策。**兩邊商店都強制要求一個公開網址**，App 內「設定」分頁也有入口。聯絡信箱已填 `getnabor@gmail.com`（`terms.html` 同一個）

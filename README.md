...
在「我的 → 事故案件」切換、結案、刪除。所有案件排成一張列表，目前這件在列表裡以勾選標記與左緣色條標示 —— **切換造成的變化必須發生在使用者正在看的地方**，否則點下去像是什麼都沒發生。結案不會刪任何東西，只是標記並自動開一件新的。任何一件都能刪，包括目前這件與唯一那一件 —— 刪到一件不剩時會自動留一件空白的，**App 不能處在「沒有任何案件」的狀態**。
